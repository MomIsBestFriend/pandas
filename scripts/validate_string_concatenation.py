...
def strings_unnecessary_split_over_lines(
    file_obj: IO,
    ...
    for first_token, second_token, third_token in zip(tokens, tokens[1:], tokens[2:]):
        if first_token.type == token.INDENT:
            indent = first_token.string

        # Check if we are dealing with string concat over a block
        if not (
        ...
        if first_string.endswith(r"\n"):
            continue

        if (
            len(indent)
            + (first_token.end[1] - first_token.start[1])
            + (third_token.end[1] - third_token.start[1])
            + max(
                get_prefix_len(first_token.string), get_prefix_len(third_token.string)
            )
        ) <= 88:
            yield third_token.start[0], " "
...
if __name__ == "__main__":
    available_validation_types: List[str] = [
        ...
        "strings_unnecessary_split_over_lines",
